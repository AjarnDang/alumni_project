...
if(isset($_POST["type"]) && $_POST["type"]=='add') {
	...
	if ($results) { //we have the product info 		
		...
		$new_product = array(array('name'=>$obj->productName,
								   'Picture'=>$Picture,
								   'code'=>$Product_ID ,
								   'qty'=>$product_qty,
								   'price'=>$obj->Price)
		...
		if(isset($_SESSION["cart_session"])) { //if we have the session		
			...
			foreach ($_SESSION["cart_session"] as $cart_itm) { //loop through session array
				if($cart_itm["code"] == $Product_ID){ //the item exist in array
					...
					$found = true;
				} else { //item doesn't exist in the list, just retrive old info and prepare array for session var
					$product[] = array('name'=>$cart_itm["name"],
									   'Picture'=>$Picture,
									   'code'=>$cart_itm["code"],
									   'qty'=>$cart_itm["qty"],
			...
			} else { //found user item in array list, and increased the quantity
				$_SESSION["cart_session"] = $product;
			}			
		} else { //create a new session var if does not exist
