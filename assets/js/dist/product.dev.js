...
horVerSlider = {
  ...
  HNav: function HNav() {
    $(".horizone-nav .next").on("click", function () {
      ...
      horVerSlider.HElement.find("ul").css({
        width: horVerSlider.zoomedWidth * horVerSlider.HElementCount * horVerSlider.ZoomeLevel,
        height: horVerSlider.zoomedWidth * horVerSlider.ZoomeLevel,
        left: -(horVerSlider.zoomedWidth * horVerSlider.ZoomeLevel * horVerSlider.HElemIndex),
        top: 0
      });

      if (horVerSlider.HMovement != -((horVerSlider.HElementCount - 1) * horVerSlider.HWidth)) {
        ...
        horVerSlider.HMovement = -(horVerSlider.HWidth * horVerSlider.VElemIndex);
  ...
  handleVertElm: function handleVertElm(elemIndex) {
    $(horVerSlider.VElement).find("li").removeClass("active");
    $(horVerSlider.VElement).find("li:eq(" + elemIndex + ")").addClass("active");
    ...
    if (elemIndex < horVerSlider.VElementCount - 3) {
      $(horVerSlider.VElement).find("ul").animate({
        top: horVerSlider.VMovement
      }, 500);
    }
  },
  moveLeft: function moveLeft() {
    $(horVerSlider.HElement).find("ul").animate({
  ...
  verticalsSliding: function verticalsSliding(verticalIndex, oldverticalIndex) {
    if (verticalIndex > 3) {
      horVerSlider.VMovement = -((verticalIndex - 3) * horVerSlider.VHeight);
      $(horVerSlider.VElement).find("ul").animate({
        top: horVerSlider.VMovement
      }, 500);
    } else if (verticalIndex < 2 && verticalIndex != 0 && oldverticalIndex > verticalIndex) {
  ...
  verticalClick: function verticalClick() {
    horVerSlider.VElement.find("li").on("click", function () {
      horVerSlider.ZoomeLevel = 1;
      horVerSlider.HElement.addClass("zoomin").removeClass("zoomout").removeClass("zoomenable");
      horVerSlider.addRemoveZoomCls(horVerSlider.ZoomeLevel);
      horVerSlider.HElement.find("li").css({
        width: horVerSlider.zoomedWidth * horVerSlider.ZoomeLevel,
        height: horVerSlider.zoomedWidth * horVerSlider.ZoomeLevel,
      ...
      $(".dotwrap > div").removeClass("active");
      $(".dotwrap > div:eq(" + horVerSlider.VElemIndex + ")").addClass("active");
      $(horVerSlider.HElement).find("ul").animate({
        left: horVerSlider.HMovement
      }, 500);
      horVerSlider.disabledNav(horVerSlider.VElemIndex);
      ...
      horVerSlider.verticalsSliding(horVerSlider.VElemIndex, horVerSlider.VElementOldIndex);
  ...
  dotClick: function dotClick() {
    $(document).on("click", ".dotwrap > div", function () {
      ...
      $(this).addClass("active");
      horVerSlider.VElement.find("li").removeClass("active");
      horVerSlider.VElement.find("li:eq(" + horVerSlider.HElemIndex + ")").addClass("active");
      horVerSlider.HMovement = -(horVerSlider.HElemIndex * horVerSlider.HWidth);
      horVerSlider.VMovement = -(horVerSlider.VElemIndex * horVerSlider.VHeight);
      $(horVerSlider.HElement).find("ul").animate({
        left: horVerSlider.HMovement
      }, 500);
      ...
      if (horVerSlider.VElemIndex < horVerSlider.VElementCount - 3) {
        ...
        horVerSlider.VMovement = -((horVerSlider.VElementCount - 4) * horVerSlider.VHeight);
        $(horVerSlider.VElement).find("ul").animate({
          top: horVerSlider.VMovement
        }, 500);
      }
  ...
  getZoomWidth: function getZoomWidth() {
    horVerSlider.winWidth = $(window).width();
    horVerSlider.winHeight = $(window).height(); //console.log(horVerSlider.winWidth + "  " + horVerSlider.winHeight);
  ...
  HtoughMove: function HtoughMove() {
    $(horVerSlider.HElement).on("touchstart", function (e1) {
      horVerSlider.HToughDown = true; //horVerSlider._dragging = true;

      horVerSlider.HtoughedAt = e1.originalEvent.touches[0].pageX;
    }).mouseup(function () {
      horVerSlider.HToughDown = false; //horVerSlider._dragging = false;
    });
  ...
  lightBox: function lightBox() {
    $("#horizon-slider").on("click", function () {
      if ($(this).hasClass("zoomenable")) {
        setTimeout(function () {
          if (!$(this).find("li:eq(" + horVerSlider.HElemIndex + ")").hasClass("video")) {
            ...
            if (horVerSlider.zoomedWidth < horVerSlider.zoomedWidth * horVerSlider.ZoomeLevel) {
              horVerSlider.HElement.find("ul").css({
                width: horVerSlider.zoomedWidth * horVerSlider.HElementCount * horVerSlider.ZoomeLevel,
                height: horVerSlider.zoomedWidth * horVerSlider.ZoomeLevel,
                left: -(horVerSlider.zoomedWidth * horVerSlider.ZoomeLevel / 4 + horVerSlider.zoomedWidth * horVerSlider.HElemIndex * horVerSlider.ZoomeLevel),
  ...
  addRemoveZoomCls: function addRemoveZoomCls(ZoomeLevel) {
    if (ZoomeLevel >= 2) {
  ...
  lightBoxZoom: function lightBoxZoom() {
    ...
    $(document).on("mouseup", ".fullscreen #horizon-slider", function () {
      if (!horVerSlider._dragging && !$(this).find("li:eq(" + horVerSlider.HElemIndex + ")").hasClass("video")) {
        horVerSlider.zoomedWidth = horVerSlider.getZoomWidth();
      ...
      $("#horizon-slider ul li").draggable({
        containment: [horizonSliderWidth + horizonSliderLeft - horVerSlider.getZoomWidth() * horVerSlider.ZoomeLevel, horizonSliderHeight + horizonSliderTop - horVerSlider.getZoomWidth() * horVerSlider.ZoomeLevel, horizonSliderLeft, horizonSliderTop]
      });
    });
  },
  zoomedMove: function zoomedMove() {
    if ($(".horVerSlider").hasClass("fullscreen")) {
      var horizonSliderLeft = $("#horizon-slider").offset().left;
      var horizonSliderTop = $("#horizon-slider").offset().top;
      var horizonSliderWidth = $("#horizon-slider").width();
  ...
  playVideo: function playVideo() {
    $("#horizon-slider li.video").on("click", function (e) {
      $(this).closest(".horVerSlider").addClass("fullscreen");
      $("body").addClass("pdppopopen");
      $("html").css("overflow", "hidden");
      var pvideo = $(this).data("pvideo");

      var _iframe = "<iframe width='560' height='315' src='" + pvideo + "' frameborder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe>";

      $(this).empty().html(_iframe).removeClass("playicon");
      $(this).css({
        backgroundImage: ""
      });
      horVerSlider.ZoomeLevel = 1;
    ...
    $(".close, .horizone-nav .next, .horizone-nav .prev").on("click", function (e) {
      $(".video").each(function () {
        $(this).empty();
        var himage = $(this).data("image");
  ...
  playThumbVideo: function playThumbVideo() {
    $("#vertical-slider li").on("click", function (e) {
      ...
      if ($(this).hasClass("video")) {
        ...
        _this.empty().html(_iframe).removeClass("playicon");
        ...
        var _left = horVerSlider.zoomedWidth * horVerSlider.ZoomeLevel * horVerSlider.HElemIndex;
  ...
  disabledNav: function disabledNav(_index) {
    horVerSlider.VElementCount = horVerSlider.VElement.find("ul li").length;
